  ...
  if SeamlessDatabasePool::TestModel.database_configs.empty?
    ...
    SeamlessDatabasePool::TestModel.database_configs.keys.each do |adapter|
      context adapter do
        ...
        let(:read_connection){ connection.available_read_connections.first.connection }
        let(:master_connection){ connection.master_connection.connection }
        ...
        it 'should work with query caching' do
          ...
          model.cache do
            ...
            connection.master_connection.connection.update("UPDATE #{model.table_name} SET value = 0 WHERE id = #{record_id}")
        ...
        context 'dead connection' do
          it 'should have two slaved' do
            connection.available_read_connections.size.should == 2
          end

          it 'should query model with dead read connection' do
            ActiveRecord::Base.logger = Logger.new(StringIO.new)
            read_pool1 = connection.available_read_connections.first
            ...
            read_pool1.should_receive(:new_connection).and_raise(StandardError.new("Can't connect"))
            SeamlessDatabasePool.use_persistent_read_connection do
              SeamlessDatabasePool.set_backup_connection_type(:master) do
                model.all
                model.where(name: 'test').count.should == 1
              end
            end
          end
        end
        ...
        context 'read connection' do
          ...
          #   columns.should == %w(id name value)
