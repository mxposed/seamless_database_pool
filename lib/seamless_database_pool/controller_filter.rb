  ...
  module ControllerFilter
    ...
    module ClassMethods
      ...
      def use_database_pool(options)
        ...
        options.each_pair do |actions, config|
          unless config.kind_of?(Hash)
            config = {:default => config}
          end

          unless SeamlessDatabasePool::READ_CONNECTION_METHODS.include?(config[:default])
            raise "Invalid read pool method: #{config[:default]}; should be one of #{SeamlessDatabasePool::READ_CONNECTION_METHODS.inspect}"
          end
          if config[:backup]
            unless SeamlessDatabasePool::READ_CONNECTION_METHODS.include?(config[:backup])
              raise "Invalid read pool method: #{config[:backup]}; should be one of #{SeamlessDatabasePool::READ_CONNECTION_METHODS.inspect}"
            end
          end
          actions = [actions] unless actions.kind_of?(Array)
          actions.each do |action|
            remapped_options[action.to_sym] = config
          end
        end
    ...
    def use_master_db_connection_on_next_request
      session[:next_request_db_connection] = {default: :master} if session
    end
    ...
    def set_read_only_connection_for_block(action)
      ...
      if read_pool_method
        SeamlessDatabasePool.set_read_only_connection_type(read_pool_method[:default]) do
          if read_pool_method[:backup]
            SeamlessDatabasePool.set_backup_connection_type(read_pool_method[:backup]) do
              yield
            end
          else
            yield
